...
void cal(void *val3)
{
  struct val *val2=(struct val*)val3;
   pthread_mutex_lock(&(val2->lockt)[ind]);
  printf("%d\n",(val2->num)[ind]);
    while(1){
        //printf("%d",trans);
	if(ind>=trans)
        { break;
   //    pthread_mutex_unlock(&(val2->lockt)[ind]);        
 }
...
{
     int fd1, ctr,start=0,count,end,i,j;
     FILE *fd2;
   ...
   /*Read the complete file into buff
	...
	//pthread_mutex_init(&lockb,NULL);
     ...
     for(ctr=0; ctr < trans; ++ctr){
       sscanf(temp[ctr],"%d %d %f %d %d",&n[ctr],&t[ctr],&a[ctr],&a1[ctr],&a2[ctr]);
        pthread_mutex_init(&lockt[ctr],NULL);
      }
     struct val val1;
     val1.num = n;
     val1.type = t;
     ...
     for(ctr=0; ctr < tds; ++ctr){
        if(pthread_create(&threads[ctr], NULL, cal, (void*)&val1) != 0){
